...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for rel in orm.Relationship.objects.filter(from_group__isnull=False):
            rel.groups.add(rel.from_group)
            rel.direct = False
            rel.save()
    ...
    def backwards(self, orm):
        ...
        for rel in orm.Relationship.objects.filter(direct=False):
            groups = rel.groups.all()
            if len(groups):
                rel.from_group = groups[0]
                rel.save()
            else:
                rel.delete()
