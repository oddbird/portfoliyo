...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for family in orm.Profile.objects.filter(invited_by__isnull=False):
            orm.TextSignup.objects.create(
                teacher=family.invited_by,
                family=family,
                group=family.invited_in_group,
                state=family.state,
                )

    def backwards(self, orm):
        "Write your backwards methods here."
        for signup in orm.TextSignup.objects.all():
            p = signup.family
            p.invited_by = signup.teacher
            p.invited_in_group = signup.group
            p.state = signup.state
            p.save()
