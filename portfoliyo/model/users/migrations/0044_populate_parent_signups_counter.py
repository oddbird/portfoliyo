...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for teacher in orm.Profile.objects.filter(school_staff=True).annotate(
                numsignups=models.Count('signed_up'),
                lastsignup=models.Max('signed_up__family__user__date_joined'),
                ).filter(numsignups__gte=1):
            timestamp = localtime(
                teacher.lastsignup).strftime('%Y-%m-%dT%H:%M:%S')
            people_set(
                teacher.user_id,
                {
                    'parentSignups': teacher.numsignups,
                    'lastParentSignup': timestamp,
                    },
                )
