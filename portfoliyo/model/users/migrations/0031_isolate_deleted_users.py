...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.Group.students.through.objects.filter(
            profile__deleted=True).delete()
        # remove deleted elders from all groups
        orm.Group.elders.through.objects.filter(
            profile__deleted=True).delete()
        # remove all relationships to deleted students
        orm.Relationship.objects.filter(to_profile__deleted=True).delete()
        # remove all relationships from deleted elders
        orm.Relationship.objects.filter(from_profile__deleted=True).delete()
