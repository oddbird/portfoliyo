...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        rels_by_members = {}
        for rel in orm['users.Relationship'].objects.select_related(
                'to_profile', 'from_profile'):
            rels_by_members[(rel.to_profile, rel.from_profile)] = rel

        for post in orm.Post.objects.exclude(author=None).select_related(
                'author', 'student'):
            rel = rels_by_members.get((post.student, post.author), None)
            if rel is not None:
